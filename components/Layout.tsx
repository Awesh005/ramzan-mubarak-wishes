...
export const Layout: React.FC<LayoutProps> = ({ children, isCentered = true }) => {
  return (
    <div className="min-h-screen w-full relative overflow-x-hidden 
                    bg-gradient-to-b from-emerald-950 via-black to-emerald-950
                    dark:from-emerald-950 dark:via-black dark:to-emerald-950
                    light:from-light-bg light:via-light-cream light:to-light-bg
                    transition-colors duration-500">
      {/* Background patterns */}
      <div className="absolute inset-0 opacity-10 dark:opacity-10 light:opacity-5 fixed" 
           style={{ 
      ...
      {/* Decorative Elements (Fixed position to stay while scrolling) */}
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <Star className="top-1/4 left-1/3" delay={0.5} size={2} />
        <Star className="bottom-1/3 right-1/2" delay={2.5} size={4} />
      </div>

      {/* Main Content */}
      <main className={`relative z-10 min-h-screen flex flex-col ${isCentered ? '' : ''}`}>
        <div className={`flex-grow flex flex-col px-4 py-12 ${isCentered ? 'items-center justify-center' : 'items-start justify-start'}`}>
          {children}
        </div>

        {/* Footer */}
        <footer className="w-full text-center py-6 
                          text-emerald-400/60 dark:text-emerald-400/60 light:text-emerald-600/70
